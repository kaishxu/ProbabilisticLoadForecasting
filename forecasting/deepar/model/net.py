    ...
    def test(self, x, v_batch, id_batch, hidden, cell, sampling=False, one_step=False):
        ...
        if sampling:
            ...
            for j in range(self.params.sample_times):
                ...
                decoder_cell = cell
                for t in range(self.params.predict_steps):  # predict_steps = 24
            ...
            sample_mu = torch.mean(samples, dim=0)
...
def accuracy_ND(mu: torch.Tensor, labels: torch.Tensor, relative = False):
    zero_index = (labels != 0)
    if relative:
        diff = torch.mean(torch.abs(mu[zero_index] - labels[zero_index])).item()  # e.g., 将tensor(23.)转为23.0
        return [diff, 1]
    else:

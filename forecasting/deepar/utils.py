...
def init_metrics(sample=True):
    metrics = {
        ...
        'test_loss': np.zeros(2),  # total loss, count
    }
    if sample:
...
def get_metrics(sample_mu, labels, predict_start, samples=None, relative=False):  # decoder结果, 产生最终结果(结果为数值)
    metric = dict()
...
def update_metrics(raw_metrics, input_mu, input_sigma, sample_mu, labels, predict_start, samples=None, relative=False):  # 在encoder结果上补充decoder结果, 不产生最终结果(结果为np数组)
    ...
    input_time_steps = input_mu.numel()  # 返回元素个数
...
def final_metrics(raw_metrics, sampling=False):
    ...
    summary_metric['test_loss'] = (raw_metrics['test_loss'][0] / raw_metrics['test_loss'][1]).item()  # 是否需要加.item(), 测试后考虑删除
    if sampling:
